...
def parse_arguments():
    ...
    parser.add_argument("--wandb-run-name", type=str, default="diffusion_run")
    parser.add_argument("--noise-type", type=str, default="linear", help = "Either 'linear', 'cosine' or 'quadratic'")
    return parser.parse_args()


if __name__=="__main__":
    args = parse_arguments()
    wandb.init(project="Clevr_diffusion", name=args.wandb_run_name, save_code=True)
    diffusion_model = DiffusionModel(device=args.device, 
                                     checkpoint_name=args.checkpoint_name, mode="train", noise_type=args.noise_type)
